/* eslint-env jest */
import { JSONSchemaDraft07 } from '@criteria/json-schema'
import fs from 'fs'
import path from 'path'
import { OutputFormat, jsonValidatorDraft07 } from '../../src'

const testCasesDirectory = path.resolve(__dirname, '../__fixtures__/json-schema-test-suite/tests/draft7')
...
const retrieveRemote = (uri: string): JSONSchemaDraft07 => {
...
describe.each(testFilesTable)(`tests/draft7/%s`, (testFilename) => {
  ...
  describe.each(testCasesTable)('%s', (testCaseDescription, testCaseSchema, testCaseTests) => {
    describe.each([
      ...
      beforeAll(() => {
        expect(() => {
          testCaseSchemaValidator = jsonValidatorDraft07(testCaseSchema, {
